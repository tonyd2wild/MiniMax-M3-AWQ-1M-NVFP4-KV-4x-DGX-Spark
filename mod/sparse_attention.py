...
class MiniMaxM3SparseMetadataBuilder(AttentionMetadataBuilder[MiniMaxM3SparseMetadata]):
    ...
    def get_cudagraph_support(cls, vllm_config, kv_cache_spec):
        # nvfp4 (EXPERIMENTAL): the scratch-dequant read is data-dependent and
        # not capturable outside the layer's eager break. Report NEVER unless
...
class MiniMaxM3SparseTritonImpl(MiniMaxM3SparseImpl):
    ...
    def forward(
        ...
        # nvfp4 (EXPERIMENTAL): the cache is packed 4-bit uint8. Dequant ONLY
        # the topk-referenced physical blocks into a bf16 scratch and remap
        # the block tables onto it; the attend kernels below run unchanged.
        ...
        if self.use_nvfp4_kv or (
            ...
            # Data-dependent: safe here because _run_attention (our caller)
            # is an eager_break_during_capture region.
