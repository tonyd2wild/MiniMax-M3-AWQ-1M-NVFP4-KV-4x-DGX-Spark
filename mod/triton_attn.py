...
class TritonAttentionMetadataBuilder(AttentionMetadataBuilder[TritonAttentionMetadata]):
    ...
    def get_cudagraph_support(cls, vllm_config, kv_cache_spec):
        # nvfp4 (EXPERIMENTAL): the scratch-dequant read in the forward is
        # data-dependent and not capturable outside an eager break. Report
        # NEVER unless explicitly allowed (VLLM_M3_NVFP4_ALLOW_CG=1,
        # breakable-CG only).
        from vllm.models.minimax_m3.common.ops.nvfp4_kv import NVFP4_ALLOW_CG
...
class TritonAttentionImpl(AttentionImpl):
    ...
    def forward(
        ...
        # nvfp4 (EXPERIMENTAL): dequant every block the batch references into
        # a bf16 scratch cache + remapped block table, then attend on the
        # scratch exactly like an unquantized cache. Data-dependent shapes:
        # runs eagerly (unified_attention_with_output is an eager break point;
        # the builder reports CG NEVER for nvfp4 unless overridden).
        if self._kv_quant_mode.is_nvfp4:
