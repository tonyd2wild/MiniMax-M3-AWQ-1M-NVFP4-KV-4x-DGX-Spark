...
class TritonAttentionMetadataBuilder(AttentionMetadataBuilder[TritonAttentionMetadata]):
    ...
    def get_cudagraph_support(cls, vllm_config, kv_cache_spec):
        # nvfp4: with the Phase-2 INLINE read path (default) the packed cache
        # is dequantized inside the attention kernel -- reads are static-shape
        # and capture-safe, so keep the native support level. Only the
        # Phase-1 SCRATCH fallback (VLLM_M3_NVFP4_INLINE=0) is data-dependent
        # and must report NEVER unless explicitly allowed
        # (VLLM_M3_NVFP4_ALLOW_CG=1, breakable-CG only).
        from vllm.models.minimax_m3.common.ops.nvfp4_kv import (
            NVFP4_ALLOW_CG,
            NVFP4_INLINE,
        )

        quant_mode = getattr(kv_cache_spec, "kv_quant_mode", None)
        if (
            quant_mode is not None
            and getattr(quant_mode, "is_nvfp4", False)
            and not NVFP4_INLINE
            and not NVFP4_ALLOW_CG
        ):
...
class TritonAttentionBackend(AttentionBackend):
    ...
    supported_kv_cache_dtypes: ClassVar[list[CacheDType]] = [
        ...
        "int8_per_token_head",
        "fp8_per_token_head",
        "nvfp4",  # EXPERIMENTAL: packed 4-bit, scratch-dequant read path
    ]
    ...
    def get_kv_cache_shape(
        ...
        if cache_dtype_str == "nvfp4":
            # EXPERIMENTAL: packed uint8, fp4 data + fp8 block scales per
            # (token, head) row (MiMo row-interleaved convention).
...
class TritonAttentionImpl(AttentionImpl):
    ...
    def __init__(
        ...
        # nvfp4 (EXPERIMENTAL): we dequant the packed KV cache into a bf16
        # scratch and run vanilla bf16 attention (kv_quant_mode=NONE), so the
        # QUERY must stay bf16. If left enabled, the generic Attention layer
        # builds a QuantFP8 query_quant for nvfp4 and hands impl.forward an
        # fp8 query -> our scratch-dequant would request output_dtype=fp8
        # (which flashinfer's nvfp4_kv_dequantize rejects: DISPATCH_DLPACK_DTYPE
        # supports only bf16/fp16) AND fp8-Q vs bf16-K would mismatch.
        # Clearing this makes the layer skip query quantization for nvfp4.
    ...
    def forward(
        ...
        block_table_arg = attn_metadata.block_table
        kv_quant_mode_arg = self._kv_quant_mode

        # nvfp4 (EXPERIMENTAL):
        # Phase 2 (default, VLLM_M3_NVFP4_INLINE=1): hand the PACKED uint8
        # K/V planes straight to unified_attention with nvfp4_packed=True;
        # the kernel dequantizes inline at the tile load (e2m1 LUT + fp8
        # block scales). No scratch pass, no block-table remap; static
        # shapes -> capture-safe. A WMMA tensor-core flash-decode fast path
        # may take over decode inside unified_attention.
        # Phase 1 fallback (VLLM_M3_NVFP4_INLINE=0): dequant every block the
        # batch references into a bf16 scratch cache + remapped block table
        # (data-dependent; runs eagerly, builder reports CG NEVER).
        nvfp4_packed_arg = False
        if self._kv_quant_mode.is_nvfp4:
            from vllm.models.minimax_m3.common.ops.nvfp4_kv import (
                NVFP4_INLINE,
                nvfp4_full_attn_scratch,
            )

            if NVFP4_INLINE:
                key_cache, value_cache = kv_cache.unbind(1)
                nvfp4_packed_arg = True
            else:
                scratch, block_table_arg = nvfp4_full_attn_scratch(
                    kv_cache, attn_metadata.block_table, query.dtype
                )
                key_cache, value_cache = scratch.unbind(1)
            kv_quant_mode_arg = KVQuantMode.NONE
            descale_shape = (
        ...
        unified_attention(
            ...
            kv_quant_mode=kv_quant_mode_arg,
            k_scale_cache=k_scale_cache,
            v_scale_cache=v_scale_cache,
            chunk_lookback=self.chunk_lookback,
            use_td=self.use_td,
            nvfp4_packed=nvfp4_packed_arg,
        )
    ...
    def do_kv_cache_update(
        ...
        if self.attn_type in (AttentionType.ENCODER_ONLY, AttentionType.ENCODER):
            ...
            # we use direct Q, K, V tensors without caching
            return
        # nvfp4 (EXPERIMENTAL): quantize + scatter into the packed 4-bit
        # cache (capturable: static shapes, in-kernel slot<0 skip).
        if self._kv_quant_mode.is_nvfp4:
            from vllm.models.minimax_m3.common.ops.nvfp4_kv import nvfp4_store_kv

            nvfp4_store_kv(key, value, kv_cache, slot_mapping)
            return
    ...
    def fused_rope_kvcache_supported(self):
        if self._is_per_token_head_quant:
            return False
        # nvfp4 (EXPERIMENTAL): the fused rope+cache op cannot write the
        # packed 4-bit layout; use do_kv_cache_update instead.
        if self._kv_quant_mode.is_nvfp4:
            return False
