...
nvfp4 (EXPERIMENTAL, Phase 2 inline dequant): when the cache is the packed
4-bit layout (uint8, last dim = head_dim//2 + head_dim//16), both kernels
dequantize IN-KERNEL at the K/V tile load: byte b of a row holds dims 2b
(low nibble) / 2b+1 (high nibble); the e2m1 nibbles are decoded via a
16-entry LUT gather and multiplied by the row's fp8-e4m3 block scales
(1 per 16 dims, at row offset head_dim//2). No bf16 scratch pass, no
block-table remap, no data-dependent host ops -> capture-safe.
"""
...
def _gqa_sparse_fwd_kernel(
    ...
    USE_NVFP4: tl.constexpr = False,  # packed nvfp4 KV: inline dequant on load
    scale_cache_ptr=None,  # fp8-e4m3 view of kv_cache (same strides)
    ...
    for j in range(real_q_loop):
        ...
        for _ in range(real_topk):
            ...
            if USE_NVFP4:
                # Inline dequant: load packed fp4 bytes [D/2, K], unpack the
                # two nibbles per byte (low nibble = even dim), LUT-decode
                # e2m1, then multiply by the per-16-dim fp8 block scale
                # (loaded once per group [D/16, K] and broadcast-expanded in
                # registers -- a full [D, K] scale load blows shared memory
                # at the 128x128 tile).
                fp4b = head_dim // 2
                off_b = tl.arange(0, BLOCK_SIZE_D // 2)
                b_mask = off_b < fp4b
                off_g = tl.arange(0, BLOCK_SIZE_D // 16)
                g_mask = off_g < (head_dim // 16)
                ...
                kb = tl.load(
                    ...
                    + pid_kh * stride_kv_h
                    + off_b[:, None] * stride_kv_d,
                    mask=b_mask[:, None] & pos_mask[None, :],
                    other=0,
                ).to(tl.int32)
                k_nib = ((kb[:, None, :] >> nib_sh[None, :, None]) & 0xF).reshape(
                    BLOCK_SIZE_D, BLOCK_SIZE_K
                )
                k_sf_g = tl.load(
                    scale_cache_ptr
                    + page * stride_kv_blk
                    ...
                    + off_n[None, :] * stride_kv_pos
                    + pid_kh * stride_kv_h
                    + (fp4b + off_g)[:, None] * stride_kv_d,
                    mask=g_mask[:, None] & pos_mask[None, :],
                    other=0.0,
                ).to(tl.float32)
                k_sf = tl.broadcast_to(
                    k_sf_g[:, None, :],
                    (BLOCK_SIZE_D // 16, 16, BLOCK_SIZE_K),
                ).reshape(BLOCK_SIZE_D, BLOCK_SIZE_K)
                k = (tl.load(lut_ptr + k_nib) * k_sf).to(q.dtype)
            else:
                k = tl.load(
                    kv_cache_ptr
                    + page * stride_kv_blk
            ...
            if USE_NVFP4:
                ...
                vb = tl.load(
                    ...
                    + pid_kh * stride_kv_h
                    + off_b[None, :] * stride_kv_d,
                    mask=pos_mask[:, None] & b_mask[None, :],
                    other=0,
                ).to(tl.int32)
                ...
                v_sf_g = tl.load(
                    ...
                    + pid_kh * stride_kv_h
                    + (fp4b + off_g)[None, :] * stride_kv_d,
                    mask=pos_mask[:, None] & g_mask[None, :],
                    other=0.0,
                ).to(tl.float32)
                v_sf = tl.broadcast_to(
                    v_sf_g[:, :, None],
                    (BLOCK_SIZE_K, BLOCK_SIZE_D // 16, 16),
                ).reshape(BLOCK_SIZE_K, BLOCK_SIZE_D)
                v = (tl.load(lut_ptr + v_nib) * v_sf).to(q.dtype)
            else:
                v = tl.load(
                    kv_cache_ptr
                    + page * stride_kv_blk
...
def minimax_m3_sparse_attn(
    ...
    # nvfp4 packed cache: uint8 with last dim != head_dim (= hd//2 + hd//16).
    use_nvfp4 = kv_cache.dtype == torch.uint8 and kv_cache.shape[-1] != head_dim
    if use_nvfp4:
        scale_cache = kv_cache.view(torch.float8_e4m3fn)
        lut = nvfp4_e2m1_lut(q.device)
    else:
        scale_cache = kv_cache  # dummy; dead branch in the kernel
        lut = q
    ...
    _gqa_sparse_fwd_kernel[grid](
        ...
        USE_FP8=use_fp8,
        USE_NVFP4=use_nvfp4,
        scale_cache_ptr=scale_cache,
        lut_ptr=lut,
        # nvfp4: the extra staged loads (fp4 bytes + scale groups) on top of
        # the 128x128 bf16 dot tiles exceed shared memory with multi-stage
        # pipelining -> single stage.
        **({"num_stages": 1} if use_nvfp4 else _sparse_attn_num_stages_kwarg()),
    )
...
def minimax_m3_sparse_attn_decode(
    ...
    _gqa_sparse_decode_kernel[grid](
        ...
        scale_cache_ptr=scale_cache,
        lut_ptr=lut,
        # nvfp4: single stage (see minimax_m3_sparse_attn).
        **({"num_stages": 1} if use_nvfp4 else _sparse_attn_num_stages_kwarg()),
        **pdl_launch,
    )
