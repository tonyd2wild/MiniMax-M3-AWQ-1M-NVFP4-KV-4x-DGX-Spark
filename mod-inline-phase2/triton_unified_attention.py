...
def kernel_unified_attention(
    ...
    # nvfp4 (EXPERIMENTAL) inline dequant: key/value_cache_ptr are the PACKED
    # uint8 planes (kv_cache[:, 0] / [:, 1], last dim = FP4B + FP4B//8 bytes:
    # fp4 data then fp8-e4m3 block scales, 1 per 16 dims).  Dequant happens
    # at the tile load: nibble unpack (low nibble = even dim) + e2m1 LUT
    # gather * block scale.  KV_QUANT_MODE stays NONE (0); the caller passes
    # bf16 Q and the S/P math is identical to the unquantized path.
    IS_NVFP4: tl.constexpr = False,
    FP4B: tl.constexpr = 0,  # head_size // 2 (fp4 data bytes per row)
    k_fp8_scale_ptr=None,  # fp8-e4m3 view of the K plane (same strides)
    v_fp8_scale_ptr=None,  # fp8-e4m3 view of the V plane (same strides)
    lut_ptr=None,  # float32[16] e2m1 decode table
):
    ...
    for j in range(loop_lo, loop_hi):
        ...
        if IS_NVFP4:
            # Inline nvfp4 dequant (no scratch): each packed row is
            # [fp4 data FP4B bytes | fp8-e4m3 block scales FP4B//8 bytes];
            # byte b holds dims 2b (low nibble) and 2b+1 (high nibble).
            slot_in_blk = seq_offset % BLOCK_SIZE
            offs_b = tl.arange(0, HEAD_SIZE_PADDED // 2)
            b_mask = tl.where(offs_b < FP4B, 1, 0).to(tl.int1)
            off_s = FP4B + offs_d // 16
            nib_sh = 4 * tl.arange(0, 2)
            # K : (HEAD_SIZE_PADDED, TILE_SIZE)
            kb = tl.load(
                key_cache_ptr
                + physical_block_idx[None, :] * stride_k_cache_0
                + kv_head_idx * stride_k_cache_2
                + offs_b[:, None] * stride_k_cache_3
                + slot_in_blk[None, :] * stride_k_cache_1,
                mask=b_mask[:, None] & tile_mask[None, :],
                other=0,
            ).to(tl.int32)
            k_nib = ((kb[:, None, :] >> nib_sh[None, :, None]) & 0xF).reshape(
            ...
            k_sf = tl.load(
                ...
                + physical_block_idx[None, :] * stride_k_cache_0
                + kv_head_idx * stride_k_cache_2
                + off_s[:, None] * stride_k_cache_3
                + slot_in_blk[None, :] * stride_k_cache_1,
                mask=dim_mask[:, None] & tile_mask[None, :],
                other=0.0,
            ).to(tl.float32)
            K_load = tl.load(lut_ptr + k_nib) * k_sf
            # V : (TILE_SIZE, HEAD_SIZE_PADDED)
            vb = tl.load(
                value_cache_ptr
                + physical_block_idx[:, None] * stride_v_cache_0
                + kv_head_idx * stride_v_cache_2
                + offs_b[None, :] * stride_v_cache_3
                + slot_in_blk[:, None] * stride_v_cache_1,
                mask=tile_mask[:, None] & b_mask[None, :],
                other=0,
            ).to(tl.int32)
            v_nib = ((vb[:, :, None] >> nib_sh[None, None, :]) & 0xF).reshape(
            ...
            v_sf = tl.load(
                ...
                + physical_block_idx[:, None] * stride_v_cache_0
                + kv_head_idx * stride_v_cache_2
                + off_s[None, :] * stride_v_cache_3
                + slot_in_blk[:, None] * stride_v_cache_1,
                mask=tile_mask[:, None] & dim_mask[None, :],
                other=0.0,
            ).to(tl.float32)
            V_load = tl.load(lut_ptr + v_nib) * v_sf
        elif USE_TD:
...
def unified_attention(
    ...
    use_td: bool = False,
    # nvfp4 (EXPERIMENTAL) inline dequant: ``k``/``v`` are the PACKED uint8
    # planes of the nvfp4 KV cache ([num_blocks, block_size, num_kv_heads,
    # head_size//2 + head_size//16]).  The kernel dequantizes them at the
    # tile load; ``kv_quant_mode`` must be NONE and ``q`` bf16/fp16.
    ...
    if nvfp4_packed:
        assert not use_td, "nvfp4 inline dequant is incompatible with use_td"
        assert kv_quant_mode == KVQuantMode.NONE, (
            "nvfp4_packed expects kv_quant_mode NONE (bf16 Q, packed KV)"
        )
        from vllm.models.minimax_m3.common.ops.nvfp4_kv import nvfp4_e2m1_lut

        k_fp8_scale = k.view(torch.float8_e4m3fn)
        v_fp8_scale = v.view(torch.float8_e4m3fn)
        nvfp4_lut = nvfp4_e2m1_lut(q.device)

        # Optional WMMA (tensor-core) flash-decode for the long-context
        # decode regime (q_len <= 3 covers decode + MTP verify). 2.3x over
        # the Triton inline path on MiMo/GB10; falls back (returns False)
        # for any unsupported shape/feature so the engine stays correct.
        if (
            ...
            try:
                from vllm.v1.attention.ops.wmma_decode_m3 import try_wmma_decode

                if try_wmma_decode(
                    q, k, v, out, seqused_k, block_table, softmax_scale,
                    num_kv_heads, head_size, block_size, sinks, softcap,
                    window_size[0], cu_seqlens_q, max_seqlen_q,
                ):
                    return
            except Exception as _wmma_e:  # pragma: no cover
                logger.warning_once(
                    "[wmma_decode_m3] fell back to Triton: %s", _wmma_e
                )
    else:
        # Dummy pointers; the IS_NVFP4 branch is dead-code eliminated.
        k_fp8_scale = k
        v_fp8_scale = v
        nvfp4_lut = q
    ...
    # nvfp4 inline dequant: tile 32 measured SLOWER than 16 for the
    # LUT-unpack path (register pressure / occupancy; same finding as the
    # MiMo diffkv mod). Clamp both.
    if nvfp4_packed:
    ...
    kernel_unified_attention[grid](
        ...
        FP4B=head_size // 2,
        k_fp8_scale_ptr=k_fp8_scale,
        v_fp8_scale_ptr=v_fp8_scale,
        lut_ptr=nvfp4_lut,
        **launch_kwargs,
    )
