...
class MiniMaxM3SparseAttention(nn.Module, AttentionLayerBase):
    ...
    def __init__(
        ...
        # nvfp4 (EXPERIMENTAL): the fused qknorm+rope+insert CUDA kernel only
        # writes bf16/fp8 caches, so the nvfp4 path calls it WITHOUT insert
        # and quantizes + scatters K/V (and stores the bf16 index key) itself.
        self.use_nvfp4_kv = self.kv_cache_dtype == "nvfp4"
    ...
    def forward(
        ...
        if self.use_nvfp4_kv:
            # nvfp4 (EXPERIMENTAL): run the fused kernel WITHOUT cache insert
            # (kv_cache=None). It still norms+ropes k and index_k in place in
            # ``qkv`` (V needs no transform) and gathers q / index_q into the
            # dedicated buffers. Then quantize+scatter K/V into the packed
            # 4-bit main cache and store the bf16 index key ourselves.
            # All ops are static-shaped with in-kernel slot<0 skip, so this
            # store path stays cudagraph-capturable like the fused insert.
            ops.fused_minimax_m3_qknorm_rope_kv_insert(
                qkv,
                self.q_norm.weight,
                self.k_norm.weight,
                ...
                self.num_idx_heads,
                None,  # slot_mapping (no insert)
                None,  # index_slot_mapping
                None,  # kv_cache
                None,  # index_cache
                0,  # block_size (unused without insert)
                q,
                index_q,
                "auto",
            )
            # qkv row: [q | k | v | index_q | index_k], all head_dim==128.
            k0 = self.q_size
            v0 = k0 + self.kv_size
            iq0 = v0 + self.kv_size
            ik0 = iq0 + self.index_q_size
            key = qkv[:, k0:v0].view(num_tokens, self.num_kv_heads, self.head_dim)
            value = qkv[:, v0:iq0].view(
                num_tokens, self.num_kv_heads, self.head_dim
            )
            index_k = qkv[:, ik0 : ik0 + self.idx_head_dim]
            nvfp4_store_kv(key, value, self.kv_cache, main_slot_mapping)
            bf16_scatter_rows(
                index_k, self.indexer.index_cache.kv_cache, index_slot_mapping
            )
        else:
            ops.fused_minimax_m3_qknorm_rope_kv_insert(
                qkv,
