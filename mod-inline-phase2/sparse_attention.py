...
class MiniMaxM3SparseBackend(AttentionBackend):
    ...
    # nvfp4 (EXPERIMENTAL): packed 4-bit cache; the Triton impl dequants the
    # topk-referenced blocks into a bf16 scratch before the dots.
    ...
    def get_kv_cache_shape(
        ...
        if cache_dtype_str == "nvfp4":
            # Packed uint8: fp4 data + fp8 block scales per (token, head) row.
            return (
                num_blocks,
                2,
                block_size,
...
class MiniMaxM3SparseMetadataBuilder(AttentionMetadataBuilder[MiniMaxM3SparseMetadata]):
    ...
    def get_cudagraph_support(cls, vllm_config, kv_cache_spec):
        ...
        # is dequantized inside the attend kernels -- reads are static-shape
        ...
        # (torch.unique + data-sized allocs) and must report NEVER unless
        # explicitly allowed (VLLM_M3_NVFP4_ALLOW_CG=1, breakable-CG only).
...
class MiniMaxM3SparseImpl(AttentionImplBase[MiniMaxM3SparseMetadata]):
    ...
    def __init__(
        ...
        *,
        ...
        # nvfp4 is quantized but NOT an fp8 cache: it must never be viewed as
        # fp8 (packed uint8, dequanted to bf16 scratch in the Triton impl).
        self.use_nvfp4_kv = kv_cache_dtype == "nvfp4"
        self.use_fp8_kv = (
            is_quantized_kv_cache(kv_cache_dtype) and not self.use_nvfp4_kv
        )
...
class MiniMaxM3SparseTritonImpl(MiniMaxM3SparseImpl):
    ...
    def forward(
        ...
        # Default (bf16/fp8) block tables; nvfp4 replaces them with remapped
        # scratch tables below.
        decode_bt = main_md.decode.block_table if main_md.decode is not None else None
        prefill_bt = (
            main_md.prefill.block_table if main_md.prefill is not None else None
        )

        # nvfp4 (EXPERIMENTAL): the cache is packed 4-bit uint8.
        # Phase 2 (default, VLLM_M3_NVFP4_INLINE=1): pass the PACKED cache
        # straight through -- the patched sparse_attn kernels detect the
        # packed shape and dequantize inline at the K/V tile load (e2m1 LUT
        # + fp8 block scales). No scratch, no block-table remap, no
        # data-dependent host ops -> capture-safe.
        # Phase 1 fallback (VLLM_M3_NVFP4_INLINE=0): dequant ONLY the
        # topk-referenced physical blocks into a bf16 scratch and remap the
        # block tables onto it; the attend kernels then see a bf16 cache.
        # Detect via the packed shape (last dim != head_dim), not just the
        # dtype string, mirroring the MiMo mod (the string can be stale).
        if self.use_nvfp4_kv or (
            kv_cache.dtype == torch.uint8 and kv_cache.shape[-1] != hd
        ):
            if not NVFP4_INLINE:
                # Data-dependent: safe here because _run_attention (our
                # caller) is an eager_break_during_capture region.
                kv_cache, decode_bt, prefill_bt = nvfp4_sparse_scratch(
                    kv_cache, main_md, decode_topk, prefill_topk, q.dtype
                )
            # else: packed cache + original block tables go straight to the
            # inline-dequant kernels below.
        elif self.use_fp8_kv:
...
def select_main_impl_cls(
    *,
    ...
    use_msa = (
        ...
        # nvfp4 (EXPERIMENTAL): only the Triton impl has the scratch-dequant
        # read path; the MSA attend cannot read the packed cache.
