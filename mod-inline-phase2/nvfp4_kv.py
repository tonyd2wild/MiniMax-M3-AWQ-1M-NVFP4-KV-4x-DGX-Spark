...
# runs eager / piecewise-only.  Only relevant for the SCRATCH read path
# (VLLM_M3_NVFP4_INLINE=0); the inline read path is capture-safe.
NVFP4_ALLOW_CG = os.environ.get("VLLM_M3_NVFP4_ALLOW_CG", "0") == "1"

# Phase 2 (inline dequant): "1" (default) = dequantize the packed nvfp4 KV
# inside the attention kernels (no bf16 scratch pass; reads are static-shape
# and capture-safe). "0" = Phase-1 behavior: scratch-dequant via flashinfer
# + stock kernels (slow but battle-tested fallback; forces attention CG
# support to NEVER unless VLLM_M3_NVFP4_ALLOW_CG=1).
NVFP4_INLINE = os.environ.get("VLLM_M3_NVFP4_INLINE", "1") == "1"

# nvfp4 e2m1 code -> float LUT (bit 3 = sign, bits 0-2 = magnitude code).
# Matches flashinfer's nvfp4_kv_quantize nibble packing (low nibble of byte
# b = dim 2b) — same table the GB10-proven MiMo wmma_decode kernel uses;
# verified by the offline round-trip test against nvfp4_kv_dequantize.
_E2M1 = [0.0, 0.5, 1.0, 1.5, 2.0, 3.0, 4.0, 6.0,
         -0.0, -0.5, -1.0, -1.5, -2.0, -3.0, -4.0, -6.0]
_NVFP4_LUTS: dict = {}


def nvfp4_e2m1_lut(device: torch.device) -> torch.Tensor:
    """float32[16] e2m1 decode table for the in-kernel LUT gather (cached)."""
    t = _NVFP4_LUTS.get(device)
    if t is None:
        t = torch.tensor(_E2M1, dtype=torch.float32, device=device)
        _NVFP4_LUTS[device] = t
    return t
